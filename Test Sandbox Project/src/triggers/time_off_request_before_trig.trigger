trigger time_off_request_before_trig on Time_Off_Request__c bulk (before insert, before update) {
    ...
    Map<Integer,Time_Off_Request__c> time_off_request_new_map = new Map<Integer,Time_Off_Request__c>();
    ...
    if(System.Trigger.new != null){
        for (Integer i=0; i< System.Trigger.new.size(); i++){
            time_off_request = System.Trigger.new[i];
            time_off_request_new_map.put(i,time_off_request);
        }
    }
    
    if(System.Trigger.old != null){
    ...
    Map<Integer,Integer> compute_number_of_days_off_map;
    if(System.Trigger.new.size() > 0){
        compute_number_of_days_off_map = ptoPackage.compute_number_of_days_off(time_off_request_new_map, true);
    }
    for (Integer i = 0; i < System.Trigger.new.size(); i++) {
        if ((System.Trigger.new[i].Start_Date__c != null) && (System.Trigger.new[i].End_Date__c != null)) {
            ...
            if ((System.Trigger.new[i].Payroll_System__c != null) && (System.Trigger.isInsert || (System.Trigger.old[i].Status__c != System.Trigger.new[i].Status__c) || (System.Trigger.old[i].Start_Date__c != System.Trigger.new[i].Start_Date__c) || (System.Trigger.old[i].End_Date__c != System.Trigger.new[i].End_Date__c))) {
                ...
                Integer number_of_days_off = compute_number_of_days_off_map.get(i);
                ...
                System.debug('*************************************************');
                System.debug('number_of_days_off : '+number_of_days_off);
                System.debug('max_number_of_days_off_in_one_request : '+max_number_of_days_off_in_one_request);
                System.debug('*************************************************');
    ...
    Map<Integer,Integer> compute_number_of_days_off_old_map = ptoPackage.compute_number_of_days_off(time_off_request_old_map, false);
    Map<Integer,Integer> compute_number_of_days_off_new_map = ptoPackage.compute_number_of_days_off(time_off_request_new_map, false);
    Integer old_num_pto_days;
    ...
    for (Integer i = 0; i < System.Trigger.new.size(); i++) {
        ...
        if ((System.Trigger.new[i].Start_Date__c != null) && (System.Trigger.new[i].End_Date__c != null) && 
            ...
            old_num_pto_days = compute_number_of_days_off_old_map.get(i);
            new_num_pto_days = compute_number_of_days_off_new_map.get(i);
            Time_Off_Info__c toi = ptoPackage.adjust_pending_time_off(tor_toi_map, old_request, System.Trigger.new[i], 
                                                                        System.Trigger.isInsert,old_num_pto_days,
                                                                        new_num_pto_days);
